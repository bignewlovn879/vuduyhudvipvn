...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>NOQUETS ON<Color=/>"):Display()
    wait(.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>ANTI CHEAT ON KHÔNG RESET×<Color=/>"):Display()
    wait(.24)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>ADMIN SCRIPT VŨ DUY HUD UPDATE KHI 35T1<Color=/>"):Display()
    wait(.29)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>ĐỘC QUYỀN BY:NGUYỄN VŨ DUY<Color=/>"):Display()
    wait(.36)
end)
...
ToggleRandom = M:AddToggle({
        Name = "Auto Quay Xương",
        Default = false,
        Flag = "Auto Farm xương",
        Save = true,
        Callback = function(Value)
